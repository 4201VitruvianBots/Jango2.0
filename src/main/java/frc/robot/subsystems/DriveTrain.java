...
public class DriveTrain extends SubsystemBase {
    ...
    private final CANSparkMax[] driveMotors = { new CANSparkMax(Constants.leftFrontDriveMotor, MotorType.kBrushless),
            new CANSparkMax(Constants.leftRearDriveMotor, MotorType.kBrushless),
            new CANSparkMax(Constants.rightFrontDriveMotor, MotorType.kBrushless),
            new CANSparkMax(Constants.rightRearDriveMotor, MotorType.kBrushless) };

    private final CANEncoder[] encoders = { driveMotors[0].getEncoder(), driveMotors[1].getEncoder(),
            driveMotors[2].getEncoder(), driveMotors[3].getEncoder() };
    ...
    public DriveTrain(PowerDistributionPanel pdp) {
        ...
        /*
         * if (RobotBase.isSimulation()) { // If our robot is simulated for(int i = 0; i
         * < 4; i++) simMotors[i] = new TalonSRX(24 + i);
         * configureCtreMotors(simMotors); simMotors[0].setSensorPhase(true);
         ...
         * DriveConstants.kDrivetrainPlant, DriveConstants.kDriveGearbox,
         * DriveConstants.kDriveGearingHigh, DriveConstants.kTrackwidthMeters,
         * Constants.DriveConstants.kWheelDiameterMeters / 2.0,
         * null);//VecBuilder.fill(0, 0, 0.0001, 0.1, 0.1, 0.005, 0.005));
         *
         * // m_drivetrainSimulator = new DifferentialDrivetrainSim( //
         * DriveConstants.kDriveGearbox, // DriveConstants.kDriveGearingHigh, // 8.28,
         * // Moment of inertia. Need to get it from the CAD model // 70.44, // 140 lbs
         * // DriveConstants.kTrackwidthMeters, //
         * Constants.DriveConstants.kWheelDiameterMeters / 2.0, //
         * VecBuilder.fill(0.001, 0.001, 0.001, 0.1, 0.1, 0.005, 0.005));
         *
         * // m_leftEncoderSim = new EncoderSim(m_leftEncoder); // m_rightEncoderSim =
         * new EncoderSim(m_rightEncoder); m_gyroAngleSim = new
         * ADXRS450_GyroSim(m_gyro); }
         */
        SmartDashboard.putData("DT Subsystem", this);
    }

    public void configureCtreMotors(CANSparkMax... motors) {
        for (int i = 0; i < motors.length; i++) {
            ...
            motors[i].setIdleMode(IdleMode.kCoast);
            ...
            // driveMotors[i].configSupplyCurrentLimit(new
            // SupplyCurrentLimitConfiguration(true, 30, 0, 0));
            // driveMotors[i].configSelectedFeedbackSensor(FeedbackDevice.IntegratedSensor);
            // } else if(motors[i] instanceof TalonSRX) {
            // simMotors[i].configSupplyCurrentLimit(new
            // SupplyCurrentLimitConfiguration(true, 30, 0, 0));
        ...
        motors[1].setOpenLoopRampRate(0);
        motors[3].setOpenLoopRampRate(0);
    }
    ...
    public double getWheelDistanceMeters(int sensorIndex) {
        ...
        if (RobotBase.isReal())
            ...
            return 0;// return (simMotors[sensorIndex].getSelectedSensorPosition() / 4096.0) *
                     // Math.PI * Units.feetToMeters(wheelDiameter);
            // if(sensorIndex == 0)
    ...
    public double getMotorInputCurrent(int motorIndex) {
        return driveMotors[motorIndex].getOutputCurrent();
    }
    ...
    public void setSpeeds(double leftSpeed, double rightSpeed) {
        driveMotors[0].getPIDController().setReference(
                leftSpeed * 60 / Constants.DriveConstants.kEncoderDistancePerPulse, ControlType.kSmartVelocity, 0,
                feedforward.calculate(leftSpeed));
        driveMotors[2].getPIDController().setReference(
                rightSpeed * 60 / Constants.DriveConstants.kEncoderDistancePerPulse, ControlType.kSmartVelocity, 0,
    ...
    public void setDriveTrainNeutralMode(int mode) {
        ...
        case 0:
        default:
            driveMotors[0].setIdleMode(IdleMode.kBrake);
            driveMotors[1].setIdleMode(IdleMode.kCoast);
            driveMotors[2].setIdleMode(IdleMode.kBrake);
            driveMotors[3].setIdleMode(IdleMode.kCoast);
            brakeMode[0] = true;
            brakeMode[1] = false;
            brakeMode[2] = true;
            brakeMode[3] = false;
    ...
    public void setDriveShifterStatus(boolean state) {
        ...
        // double gearRatio = state ? Constants.DriveConstants.kDriveGearingHigh :
        // Constants.DriveConstants.kDriveGearingLow ;
        // double kEncoderDistancePerPulse = state ?
        // Constants.DriveConstants.kEncoderDistancePerPulseHigh :
        // Constants.DriveConstants.kEncoderDistancePerPulseLow;

        // m_drivetrainSimulator.setCurrentGearing(gearRatio);
        // m_leftEncoder.setDistancePerPulse(kEncoderDistancePerPulse);
        // m_rightEncoder.setDistancePerPulse(kEncoderDistancePerPulse);

        // driveTrainShifters.set(state ? DoubleSolenoid.Value.kForward :
    ...
    public DifferentialDriveWheelSpeeds getSpeeds() {
        ...
        if (RobotBase.isReal()) {
            ...
            leftMetersPerSecond = (encoders[0].getVelocity() / 60.0)
                    * Constants.DriveConstants.kEncoderDistancePerPulse;
            rightMetersPerSecond = (encoders[2].getVelocity() / 60.0)
                    * Constants.DriveConstants.kEncoderDistancePerPulse;
        } else {
            ...
            // leftMetersPerSecond = (simMotors[0].getSelectedSensorVelocity() * 10.0 /
            // 4096.0) * Math.PI * Units.feetToMeters(wheelDiameter);
            // rightMetersPerSecond = (simMotors[2].getSelectedSensorVelocity() * 10.0 /
            // 4096.0) * Math.PI * Units.feetToMeters(wheelDiameter);
        }
    ...
    public double getTravelDistance() {
        ...
        if (RobotBase.isReal()) {
            leftMeters = 0;// encoders[0].getPosition() * gearRatio * Math.PI *
                           // Units.feetToMeters(wheelDiameter);
            rightMeters = 0;// encoders[2].getPosition() * gearRatio * Math.PI *
                            // Units.feetToMeters(wheelDiameter);
            return (leftMeters + rightMeters) / 2.0;
        } else {
            // leftMeters = m_leftEncoder.getDistance();
            // rightMeters = m_rightEncoder.getDistance();
            leftMeters = 0;// (simMotors[0].getSelectedSensorPosition() * 10.0 / 4096) * Math.PI *
                           // Units.feetToMeters(wheelDiameter);
            rightMeters = 0;// (simMotors[2].getSelectedSensorPosition() * 10.0 / 4096) * Math.PI *
                            // Units.feetToMeters(wheelDiameter);
            return (leftMeters + rightMeters) / 2.0;
        }
    }
    ...
    private void initShuffleboardValues() {
        ...
        Shuffleboard.getTab("Drive Train").addNumber("Angle", () -> getRobotPose().getRotation().getDegrees());
    ...
    private void updateSmartDashboard() {
        if (RobotBase.isReal()) {
            ...
            SmartDashboardTab.putNumber("DriveTrain", "rightSpeed",
                    Units.metersToFeet(getSpeeds().rightMetersPerSecond));
            SmartDashboardTab.putBoolean("DriveTrain", "high gear", getDriveShifterStatus());
    ...
    public void simulationPeriodic() {
        ...
        // SmartDashboard.putNumber("L Encoder Count",
        // simMotors[0].getSelectedSensorPosition());
        // SmartDashboard.putNumber("R Encoder Count",
        // simMotors[2].getSelectedSensorPosition());
        // SmartDashboard.putNumber("L Encoder Rate",
        // simMotors[0].getSelectedSensorVelocity());
        // SmartDashboard.putNumber("R Encoder Rate",
        // simMotors[2].getSelectedSensorVelocity());
